...
export default function PridatNajemnikaForm({ jednotkaId, osoby }: { jednotkaId: string; osoby: Osoba[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('najemnici').insert({
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-5">
      ...
      {rezim === 'vybrat' ? (
        <div>
          <label className={labelClass}>Vyberte nájemníka *</label>
          {osoby.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="email" type="email" value={novaOsoba.email} onChange={handleNovaOsobaChange} className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>Telefon</label>
              <input name="telefon" value={novaOsoba.telefon} onChange={handleNovaOsobaChange} className={inputClass} />
            </div>
          </div>
          <div>
      ...
      <div>
        <label className={labelClass}>Nájemník od *</label>
      ...
      <div className="flex gap-3 pt-1">
        <button type="submit" disabled={nacitani} className="bg-blue-600 text-white text-sm px-6 py-2.5 rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:opacity-50">
          {nacitani ? 'Ukládám...' : 'Přiřadit nájemníka'}
        </button>
